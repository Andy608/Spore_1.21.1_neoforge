...
public class WeaponSyringe extends AbstractSyringe{
    ...
    public boolean overrideStackedOnOther(ItemStack stack, Slot slot, ClickAction clickAction, Player player) {
        ...
        if (itemStack.getItem() instanceof SporeWeaponData weaponData && clickAction == ClickAction.SECONDARY){
            player.playNotifySound(Ssounds.SYRINGE_INJECT.value(), SoundSource.AMBIENT,1F,1F);
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        ...
        tooltipComponents.add(Component.literal(Component.translatable("spore.item.mutation").getString()+Component.translatable(mutations.getName()).getString()));
    }
}
