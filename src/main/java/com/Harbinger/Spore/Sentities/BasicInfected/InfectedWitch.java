...
public class InfectedWitch extends Infected implements RangedAttackMob , RangedBuff, EvolvingInfected {
    ...
    protected void registerGoals() {
        super.registerGoals();
        this.goalSelector.addGoal(1, new UseItemGoal<>(this, PotionContents.createItemStack(Items.POTION, Potions.STRONG_HEALING), SoundEvents.WITCH_DRINK, (p_35882_) -> {
        ...
        this.goalSelector.addGoal(1, new UseItemGoal<>(this, PotionContents.createItemStack(Items.POTION, Potions.LONG_FIRE_RESISTANCE), SoundEvents.WITCH_DRINK, (p_35882_) -> {
        ...
        this.goalSelector.addGoal(4, new BuffAlliesGoal(this,Infected.class,1.3,35,45,3, entity -> {
            return entity.hasEffect(Seffects.STARVATION) && !(entity instanceof InfectedWitch);
        }){
            @Override
            public boolean canUse() {
    ...
    public void tick() {
        super.tick();
        if (tickCount % 30 == 0  && potion != null){
            ItemStack potionStack = PotionContents.createItemStack(Items.POTION,potion);
            if (isAlive() && !this.getMainHandItem().equals(potionStack)){
            ...
            double d3 = Math.sqrt(d0 * d0 + d2 * d2);
             if (d3 >= 4.0D && getAttackPotion() != null) {
                potion = getAttackPotion();
                 ...
                 potion = entity.getType().is(EntityTypeTags.UNDEAD) ? Potions.HEALING: Potions.HARMING;
             }
